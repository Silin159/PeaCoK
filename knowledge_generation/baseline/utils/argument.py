...
def verify_args(args, parser):
    if args.eval_only:
        ...
        if not args.params_file:
            logger.info("params_file is not set, default to use the params-bert-base.json in checkpoint")
            args.params_file = os.path.join(args.checkpoint, "params-bert-base.json")
        else:
...
def update_additional_params(params, args):
    ...
    for key in ["max_tokens", "knowledge_max_tokens"]:
        if args.get(key, -1) > -1:
...
def set_default_dataset_params(args):
    ...
    set_attr_if_not_exists(args, "max_utterances", 20)
    set_attr_if_not_exists(args, "max_tokens", 512)
