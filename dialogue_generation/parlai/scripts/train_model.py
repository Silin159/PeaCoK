...
.. code-block:: shell
  ...
  python -m parlai.scripts.train -m ir_baseline -t dialog_babi:Task:1 -mf /runs/model
  python -m parlai.scripts.train -m seq2seq -t babi:Task10k:1 -mf '/runs/model' -bs 32 -lr 0.5 -hs 128
  python -m parlai.scripts.train -m drqa -t babi:Task10k:1 -mf /runs/model -bs 10
    ...
    def validate(self):
        ...
        if self.best_valid is None or self.valid_optim * new_valid[-1] > self.valid_optim * self.best_valid:
            ...
            if opt.get('model_file') and is_primary_worker():
                ...
                self.saved = True
            if (opt['validation_metric'].split(",")[-1] == 'accuracy' and
